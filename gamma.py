...
def choice(image,gamma=1):  #realiza un barrido para multiples valores de gamma, para corregir efecto de luz y extraer pantalla
    ...
    if len(n) > 17 or n[0]==0 or cv2.contourArea(c)>1000000 or cv2.contourArea(c)<700000: #Defino caracteristicas que me ayudan a definir si el contorno corresponde o no a una pantalla, dependiendo de las dimensiones de esta, y numero de esquinas,
        if gamma>24:
            return 5 #caso que no se encuentre el valor para evitar divergencia
        return choice(image,gamma+0.05)#si no se encuentra, se vuelve a ejecutar la funcion  con un un nuevo valor
    else:
        return gamma
        
def recorte(image,gamma):
